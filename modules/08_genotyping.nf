...
*/
process GENOTYPING {
    publishDir "${params.outdir}/08_GENOTYPING", mode: 'copy', pattern: '{GLOBAL_Genotyping_Report.pdf,SUMMARY_Multi_Infection.tsv,validated_consensus_all.fasta,renamed_consensus/*.fasta,renamed_consensus/manifest.tsv}'
    publishDir "${params.outdir}/00_SUPDATA", mode: 'copy', pattern: 'DB/*'
    input:
    path 'consensus_inputs/*'
    path ref_fasta
    path 'csvs/*'
    output:
