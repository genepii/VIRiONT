...
# Rend automatiquement le script R exécutable s'il ne l'est pas déjà
if [ ! -x "$R_SCRIPT" ]; then
    chmod +x "$R_SCRIPT"
fi
...
tail -n +2 "$CSV_FILE" | while IFS=';' read -r plate_pos sample component forward reverse flowcell kit_id primers; do
    ...
    echo "--> [05_FINAL_GENOTYPING] Analyse BLAST & Comptage des Reads..."
    
    RAW_COUNT_TABLE="${GENOTYPING_DIR}/${sample_id}_raw_cluster_counts.tsv"
    ...
    # --- ÉTAPE 06 : Agrégation & Filtre Co-Infections (AWK) ---
    VALIDATED_SUMMARY="${GENOTYPING_DIR}/${sample_id}_validated_genotypes.tsv"
    ...
    awk -F'\t' -v cutoff="$MI_CUTOFF" -v min_r="$MIN_READS" '
        ...
        END {
            ...
            printf "sample\tgenotype\tbest_cluster\ttotal_reads\tratio_percent\tpident\tlength\tstatus\n"
    ...
    PDF_REPORT="${GENOTYPING_DIR}/${sample_id}_genotype_report.pdf"
    ...
    grep -w "VALIDATED" "$VALIDATED_SUMMARY" | while IFS=$'\t' read -r s_id geno cluster rcount ratio pident length status; do
        ...
        MASTER_FASTA="${GENOTYPING_DIR}/${sample_id}_${geno}_master_consensus.fasta"
